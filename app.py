...
def analyze_conversation(messages, patient_data=None):
    """
    ...
    if client is None:
        return {
            "ready": False,
            "next_question": "The AI service is not configured. Please add GROQ_API_KEY in Streamlit Secrets.",
            "language": "Unknown",
            "category": "General Administration",
            "department": "Administration",
            "priority": "Medium",
            "summary": "",
            "missing_information": ["GROQ_API_KEY"],
            "safety_flag": False,
            "safety_message": "",
            "error": "GROQ_API_KEY was not found in Streamlit Secrets or environment variables.",
        }
    ...
    context_text = (
        f"Hospital already provided by the patient form: {patient_context.get('hospital', '') or 'Not provided'}\n"
        f"Department/location already provided by the patient form: {patient_context.get('location', '') or 'Not provided'}\n"
        f"Patient/complainant name: {patient_context.get('patient_name', '') or 'Not provided'}\n"
    )

    system_prompt = f"""
You are the conversational AI agent for a hospital patient grievance system in Pakistan.

The following information came from the form BEFORE this conversation. Treat it as already known and do not ask the patient for it again:
{context_text}
...
NEVER use a vague question such as "Could you please provide a little more information about what happened?"
Ask a specific question about ONE missing item, such as the hospital name, exact department/ward,
approximate date/time, or what service/action was delayed.

Information supplied in the form context above counts as known information.
...
"""
    ...
    try:
        ...
        return {
            "ready": False,
            "next_question": "I could not connect to the AI service. Please check the Groq API configuration.",
            "language": "Unknown",
            "category": "General Administration",
            "department": "Administration",
            "priority": "Medium",
...
st.sidebar.title("🏥 AI Hospital Complaint Agent")
...
if page == "Patient - AI Complaint":
    ...
    st.title("🏥 AI Hospital Complaint Assistant")

    st.write(
        "Tell me what happened. You can write in **English, Urdu, "
    ...
    if not st.session_state.patient_started:
        ...
        st.info(
            "Example: **Meri ammi emergency mein hain aur 2 ghantay "
            "se doctor nahi aya.**"
        )
        ...
        if start:
            ...
            if not hospital.strip():
                ...
                if result.get("error"):
                    st.error("AI configuration/API error: " + str(result["error"]))
